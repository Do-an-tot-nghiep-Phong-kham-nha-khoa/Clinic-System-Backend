...
exports.list = async (req, res) => {
  try {
    ...
    // Base conditions
    const conditions = {
      ...(query.id && isValidObjectId(query.id) && { _id: new mongoose.Types.ObjectId(query.id) }),
      ...(query.treatmentId && isValidObjectId(query.treatmentId) && {
      ...
      ...(query.invoiceNumber && { invoiceNumber: { $regex: query.invoiceNumber, $options: "i" } }),
    };
    ...
    if (query.q && query.q.trim()) {
      ...
      searchConditions = {
        ...conditions,
        healthProfile_id: { $in: healthProfileIds.length ? healthProfileIds : [null] },
      };
    }
    ...
    const dataQuery = Invoice.find(searchConditions)
      .populate({
        path: "treatmentId",
        select: "laborder prescription totalCost appointment healthProfile doctor treatmentDate diagnosis",
        populate: [
          { path: "laborder", populate: { path: "items.serviceId", model: "Service", select: "name price" } },
          { path: "prescription", populate: { path: "items.medicineId", model: "Medicine", select: "name price manufacturer unit expiryDate" } },
    ...
    const [data, total] = await Promise.all([dataQuery.exec(), Invoice.countDocuments(searchConditions)]);

    const resolved = await Promise.all(
      data.map(async (inv) => {
        let hp = inv.healthProfile_id || (inv.treatmentId && inv.treatmentId.healthProfile) || null;
...
exports.getById = async (req, res) => {
  try {
    ...
    if (!invoiceId || !isValidObjectId(invoiceId)) return res.status(400).json({ message: "Valid Invoice ID is required" });
    ...
    const invoice = await Invoice.findById(invoiceId)
      .populate({
        ...
        populate: [
          { path: "laborder", populate: { path: "items.serviceId", model: "Service", select: "name price" } },
          { path: "prescription", populate: { path: "items.medicineId", model: "Medicine", select: "name price" } },
    ...
    if (!invoice) return res.status(404).json({ message: `Invoice not found with ID: ${invoiceId}` });

    const hp = invoice.healthProfile_id || (invoice.treatmentId && invoice.treatmentId.healthProfile) || null;
    const owner_detail = await resolveOwnerDetail(hp);
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    const updatedInvoice = await Invoice.findByIdAndUpdate(invoiceId, { status }, { new: true, runValidators: true }).lean();
    if (!updatedInvoice) return res.status(404).json({ message: `Invoice not found with ID: ${invoiceId}` });
...
exports.createVnPayPayment = async (req, res) => {
  try {
    ...
    if (!bookerId) {
      return res.status(400).json({ message: "Cannot find appointment booker" });
    }

    // Lấy accountId của booker
    const bookerPatient = await Patient.findById(bookerId).select("accountId").lean();
    if (!bookerPatient) {
    ...
    const paymentUrl = vnpayService.buildPaymentUrl({
      amount: invoice.totalPrice,
      orderId: txnRef,
      orderInfo: `Thanh toan hoa don ${invoice.invoiceNumber || invoiceId.slice(-8)}`,
...
exports.vnpayReturn = async (req, res) => {
  try {
    ...
    if (!isValid) {
      ...
      return res.redirect(`${vnpayConfig.frontend_Url}/patient/payment-result?status=error&message=${encodeURIComponent('Chữ ký không hợp lệ')}`);
    }

    const { vnp_TxnRef, vnp_ResponseCode, vnp_TransactionNo, vnp_BankCode, vnp_CardType } = vnp_Params;
    ...
    if (!invoice) {
      ...
      const vnpayConfig = require('../../config/vnpay');
      return res.redirect(`${vnpayConfig.frontend_Url}/patient/payment-result?status=error&message=${encodeURIComponent('Không tìm thấy hóa đơn')}`);
    }
    
    console.log('✅ Invoice found:', invoice._id);

    // Update payment record
    const paymentIndex = invoice.payments.findIndex(p => p.providerPaymentId === vnp_TxnRef);
    
    if (paymentIndex !== -1) {
      invoice.payments[paymentIndex].status = vnp_ResponseCode === '00' ? 'success' : 'failed';
    ...
    res.redirect(`${vnpayConfig.frontend_Url}/patient/payment-result?status=${status}&message=${encodeURIComponent(message)}&invoiceId=${invoice._id}`);

  } catch (error) {
    console.error('Error handling VNPay return:', error);
    const vnpayConfig = require('../../config/vnpay');
    res.redirect(`${vnpayConfig.frontend_Url}/patient/payment-result?status=error&message=${encodeURIComponent('Lỗi hệ thống')}`);
  }
};
...
exports.vnpayIPN = async (req, res) => {
  try {
    ...
    const payment = invoice.payments.find(p => p.providerPaymentId === vnp_TxnRef);
...
exports.getInvoicesForAuthenticatedPatient = async (req, res) => {
  try {
    ...
    const treatments = await Treatment.find({ appointment: { $in: appointmentIds } }).select('_id healthProfile');
